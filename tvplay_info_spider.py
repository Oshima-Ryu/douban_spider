...
tvplay_sample_col = doubanDB['tvplay_sample']
tvplay_info_col = doubanDB['tvplay_info']
...
def log_record(status_code, url):
    # 日志
    f = open('data/tv_info_record.txt', 'a', encoding='utf-8')
    f.write(str(status_code) + ',' + url + ',' + str(datetime.datetime.now()) + '\n')
    f.close()


def extract_tvplay_info(page_html):
    ...
    if len(content_soup) == 1:
        ...
        try:
            release_date = movie_info_soup.find('span', text='首播:').find_next_sibling("span").text
        except:
            release_date = ''

        try:
            episode = movie_info_soup.find('span', text='集数:').next_sibling.replace(' ', '')
        except:
            episode = ''

        try:
            duration = movie_info_soup.find('span', text='单集片长:').next_sibling.replace(' ', '')
        except:
            duration = ''
        ...
        try:
            ...
            if len(interests_soup) == 3:
                watching_people = re.sub('\D', '',interests_soup[0].text)
                watched_peolpe = re.sub('\D', '',interests_soup[1].text)
                interest_peolpe = re.sub('\D', '',interests_soup[2].text)
            else:
                watching_people = 0
                watched_peolpe = 0
                interest_peolpe = 0
        ...
        tvplay_info_dict = {}
        tvplay_info_dict['json_info'] = movie_json_info
        tvplay_info_dict['movie_name'] = movie_name
        tvplay_info_dict['movie_year'] = movie_year
        tvplay_info_dict['director'] = director_list
        tvplay_info_dict['author'] = author_list
        tvplay_info_dict['actor'] = actor_list
        tvplay_info_dict['type'] = type_list
        tvplay_info_dict['state'] = state_list
        tvplay_info_dict['language'] = language_list
        tvplay_info_dict['release_date'] = release_date
        tvplay_info_dict['episode'] = episode
        tvplay_info_dict['duration'] = duration
        tvplay_info_dict['another_name'] = another_name_list
        tvplay_info_dict['rate_num'] = rate_num
        tvplay_info_dict['rate_people'] = rate_people
        tvplay_info_dict['star5_pre'] = star5_pre
        tvplay_info_dict['star4_pre'] = star4_pre
        tvplay_info_dict['star3_pre'] = star3_pre
        tvplay_info_dict['star2_pre'] = star2_pre
        tvplay_info_dict['star1_pre'] = star1_pre
        tvplay_info_dict['recommendations'] = recommendations_list
        tvplay_info_dict['comments_num'] = comments_num
        tvplay_info_dict['reviews_num'] = reviews_num
        tvplay_info_dict['tag'] = tag_list
        tvplay_info_dict['watching_people'] = watching_people
        tvplay_info_dict['watched_peolpe'] = watched_peolpe
        tvplay_info_dict['interest_peolpe'] = interest_peolpe
        tvplay_info_dict['summary'] = summary

        return tvplay_info_dict


def get_tvplay_html(url):
    global ip_ports
    agent = get_agent()
    headers = {
...
def get_tvplay_sample_list(year):
    movie_sample_cursor = tvplay_sample_col.find({'year': year})
    movie_sample_list = []
...
def save_in_tvplay_sample_col(tvplay_id_id):
    crawl_time = datetime.datetime.now()
    tvplay_sample_col.update_one({'id': tvplay_id_id}, {'$set': {'isCrawl': 1, 'crawl_time': crawl_time}})


def save_tvplay(tvplay_info_dict):
    tvplay_info_col.insert_one(tvplay_info_dict)

    #在movie_sample中记录爬取状态与时间
    tvplay_id = tvplay_info_dict['id']
    save_in_tvplay_sample_col(tvplay_id)


def start_crawl(year):
    # year = 2017
    tvplay_sample_list = get_tvplay_sample_list(year)
    for movie_sample_info in tvplay_sample_list:
        print(movie_sample_info['id'], movie_sample_info['name'] + " start")
        movie_html = get_tvplay_html(movie_sample_info['url'])
        if movie_html == 404:
            continue
        if movie_html == None:
            break
        movie_info_dict = extract_tvplay_info(movie_html)
...
if __name__ == "__main__":
    for year in [2013, 2012, 2011, 2010, 2009, 2008]:
        start_crawl(year)
